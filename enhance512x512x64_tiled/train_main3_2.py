...
class denseblock(nn.Module):
    def __init__(self, nb_filter=16, filter_wh=5):
        ...
        self.conv2_0 = nn.Conv3d(in_channels=self.conv1_0.out_channels, out_channels=self.nb_filter,
                                 kernel_size=self.nb_filter_wh, padding=self.padding)
        self.conv1_1 = nn.Conv3d(in_channels=nb_filter + self.conv2_0.out_channels, out_channels=self.nb_filter * 4,
                                 kernel_size=1)
        self.conv2_1 = nn.Conv3d(in_channels=self.conv1_1.out_channels, out_channels=self.nb_filter,
                                 kernel_size=self.nb_filter_wh, padding=self.padding)
        self.conv1_2 = nn.Conv3d(in_channels=nb_filter + self.conv2_0.out_channels + self.conv2_1.out_channels,
                                 out_channels=self.nb_filter * 4, kernel_size=1)
        self.conv2_2 = nn.Conv3d(in_channels=self.conv1_2.out_channels, out_channels=self.nb_filter,
                                 kernel_size=self.nb_filter_wh, padding=self.padding)
        self.conv1_3 = nn.Conv3d(
            ...
            out_channels=self.nb_filter * 4, kernel_size=1)
        self.conv2_3 = nn.Conv3d(in_channels=self.conv1_3.out_channels, out_channels=self.nb_filter,
                                 kernel_size=self.nb_filter_wh, padding=self.padding)
        ...
        self.batch_norm1_3 = nn.BatchNorm3d(
...
class DD_net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv3d(in_channels=INPUT_CHANNEL_SIZE, out_channels=self.nb_filter, kernel_size=(7, 7, 7),
                               padding=(3, 3, 3))
        self.dnet1 = denseblock(self.nb_filter, self.filter_wh)

        self.conv2 = nn.Conv3d(in_channels=self.conv1.out_channels * (db_num_layers + 1), out_channels=self.nb_filter,
                               kernel_size=(1, 1, 1))
        self.dnet2 = denseblock(self.nb_filter, self.filter_wh)
        self.conv3 = nn.Conv3d(in_channels=self.conv2.out_channels * (db_num_layers + 1), out_channels=self.nb_filter,
                               kernel_size=(1, 1, 1))
        self.dnet3 = denseblock(self.nb_filter, self.filter_wh)
        self.conv4 = nn.Conv3d(in_channels=self.conv3.out_channels * (db_num_layers + 1), out_channels=self.nb_filter,
                               kernel_size=(1, 1, 1))
        self.dnet4 = denseblock(self.nb_filter, self.filter_wh)

        self.conv5 = nn.Conv3d(in_channels=self.conv4.out_channels * (db_num_layers + 1), out_channels=self.nb_filter,
                               kernel_size=(1, 1, 1))

        self.convT1 = nn.ConvTranspose3d(in_channels=self.conv4.out_channels + self.conv4.out_channels,
                                         out_channels=2 * self.nb_filter, kernel_size=self.kernel_size,
                                         padding=self.padding)
        self.convT2 = nn.ConvTranspose3d(in_channels=self.convT1.out_channels, out_channels=self.nb_filter,
                                         kernel_size=1)
        self.convT3 = nn.ConvTranspose3d(in_channels=self.convT2.out_channels + self.conv3.out_channels,
                                         out_channels=2 * self.nb_filter, kernel_size=self.kernel_size,
                                         padding=self.padding)
        self.convT4 = nn.ConvTranspose3d(in_channels=self.convT3.out_channels, out_channels=self.nb_filter,
                                         kernel_size=1)
        self.convT5 = nn.ConvTranspose3d(in_channels=self.convT4.out_channels + self.conv2.out_channels,
                                         out_channels=2 * self.nb_filter, kernel_size=self.kernel_size,
                                         padding=self.padding)
        self.convT6 = nn.ConvTranspose3d(in_channels=self.convT5.out_channels, out_channels=self.nb_filter,
                                         kernel_size=1)
        self.convT7 = nn.ConvTranspose3d(in_channels=self.convT6.out_channels + self.conv1.out_channels,
                                         out_channels=2 * self.nb_filter, kernel_size=self.kernel_size,
                                         padding=self.padding)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(0, req_vol):
            if (i % stride_vol == 0):
                ...
                if (idx_rem == 0):
                    image_target = image_target[:, :, 0:out_size_image + pad, 0:out_size_image + pad].type(
                        torch.FloatTensor)
                    image_input = image_input[:, :, 0:out_size_image + pad, 0:out_size_image + pad].type(
                        torch.FloatTensor)
                elif (idx_rem == 1):
                    image_target = image_target[:, :, 0:out_size_image + pad, out_size_image - pad:].type(
                        torch.FloatTensor)
                    image_input = image_input[:, :, 0:out_size_image + pad, out_size_image - pad:].type(
                        torch.FloatTensor)
                elif (idx_rem == 2):
                    image_target = image_target[:, :, out_size_image - pad:, 0:out_size_image + pad].type(
                        torch.FloatTensor)
                    image_input = image_input[:, :, out_size_image - pad:, 0:out_size_image + pad].type(
                        torch.FloatTensor)
                else:
                    image_target = image_target[:, :, out_size_image - pad:, out_size_image - pad:].type(
                        torch.FloatTensor)
                    image_input = image_input[:, :, out_size_image - pad:, out_size_image - pad:].type(
                        torch.FloatTensor)
...
def dd_train(gpu, args):
    ...
    # trainset = VolCTDataset(root_dir='/groups/synergy_lab/garvit217/data_3d_1/train')
    # testset = VolCTDataset(root_dir='/groups/synergy_lab/garvit217/data_3d_1/test')
    # valset = VolCTDataset(root_dir='/groups/synergy_lab/garvit217/data_3d_1/validate')
    ...
    for i in range(90, 1, -10):
        # model_file_CP = "weights" + "_" + str(batch) + "_" + str(i) + "_" + str(rank) + ".pt"
    ...
    if (not (path.exists(model_file))):
        ...
        for k in range(start_epoch, epochs):
            ...
            for batch_index, batch_samples in enumerate(train_loader):
                ...
                file_name, HQ_img, LQ_img, img_max, img_min = batch_samples['vol'], batch_samples['HQ'], batch_samples[
                    'LQ'], batch_samples['max'], batch_samples['min']
            ...
            for batch_index1, batch_samples1 in enumerate(val_loader):
                file_name, HQ_img, LQ_img, img_max, img_min = batch_samples1['vol'], batch_samples1['HQ'], \
                                                              batch_samples1['LQ'], batch_samples1['max'], \
                                                              batch_samples1['min']
